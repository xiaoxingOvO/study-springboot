...
public class MyAspectConfig {
    ...
    // 3.基于注解的切入点表达式 @annotation(com.xx.annotations.xxx)

    //@Around("execution(* com.xx.service.*.*(..))")
    //@Around("within(com.xx.service.*)")
    @Around("@annotation(com.xx.annotations.MyAdvice)")
    ...
    //@Before("execution(* com.xx.service.*.*(..))") //@Before 代表这是一个核心业务逻辑执行之前附加操作  value:用来书写切入点表达式 配置附加操作在哪里生效
